/*! \file mcpcan.c
    \brief Definicije funkcija 
    
 Definicije funkcija za MCP 2515 deklarisane u h fajlu
*/

// FOSC
#pragma config FOSFPR = HS2_PLL4        // Oscillator (HS2 w/PLL 4x)
...
volatile   uint8_t   m_nExtFlg=0;                                                                                                                // Extended (29 bit) or Standard (11 bit)
volatile   uint32_t  m_nID=5;                                               
volatile   uint8_t   m_nDlc=8;                                                   
volatile   uint8_t   m_nDta[8]={0};                           
volatile   uint8_t   m_nRtr=0;                                                     
volatile   uint8_t   m_nfilhit=0;                                                  
volatile   uint8_t   MCPCS=2;                                                    
volatile   uint8_t   mcpMode=0; 

 /// obrnuti su pinovi na MCP2515 !!!!!!!!!!!!11
...
void InitSPI (void)
{
    ...
    unsigned int SPISTATValue;
                            ...
                            ///FRMEN:  0 = Framed SPI support disabled
    SPICONValue =           FRAME_ENABLE_OFF & 
                            ///SPIFSD: 0 = Frame sync pulse output (master)
                            FRAME_SYNC_OUTPUT & 
                            ...
                            SPI_SMP_OFF &     
                            ///CKE: //ovo treba   0 = Serial output data changes on transition from Idle clk state to active clk state
                            SPI_CKE_ON & 
                            ///SSEN:   0 = SS pin not used by module. Pin controlled by port function
                            SLAVE_ENABLE_OFF  &  
                            ///CKP:    0 = SS pin not used by module. Pin controlled by port function
                            CLK_POL_ACTIVE_HIGH &  
                            ///MSTEN:  1 = Master mode
                            MASTER_ENABLE_ON &  
                            ///SPRE<2:0>: Secondary Prescale 3:1
                            SEC_PRESCAL_4_1 & //// da probam da prabcin na 5 :1  i rp
                            ///PPRE<1:0> Primary Prescale 1:1
                            PRI_PRESCAL_1_1;  // da probam da prabcin na 2 :1 
                            ///SPIEN:   1 = Enables module and configures SCKx, SDOx, SDIx and SSx as serial port pins    
    SPISTATValue =          SPI_ENABLE & 
                            ///SPISIDL: 0 = Continue module operation in Idle mode
                            SPI_IDLE_CON & 
                            ///SPIROV:  0 = No overflow has occurred. Clear receive overflow bit.
                            SPI_RX_OVFLOW_CLR;     
...
void delayMicroseconds(int kas)
{
    int sec,sec1;
    for(sec=0;sec<16;sec++)
        for(sec1=0;sec1<kas;sec1++);
}
...
*********************************************************************************************************/
INT8U clearMsg()// can id podesavanja
{
    m_nID       = 0;
    m_nDlc      = 0;
    m_nExtFlg   = 0;
